...
import com.futurebytedance.marketanalysis.beans.AdClickEvent;
import com.futurebytedance.marketanalysis.beans.AdCountViewByProvince;
import com.futurebytedance.marketanalysis.beans.BlackListUserWarning;
...
//        URL resource = AdStatisticsByProvince.class.getResource("/AdClickLog.csv");
        String path = "C:\\Users\\10926\\IdeaProjects\\UserBehaviorAnalysis\\MarketAnalysis\\src\\main\\resources\\AdClickLog.csv";
        DataStream<AdClickEvent> adClickEventStream = env.readTextFile(path)
                .map(new MapFunction<String, AdClickEvent>() {
                    @Override
                    public AdClickEvent map(String value) {
        ...
        SingleOutputStreamOperator<AdCountViewByProvince> adCountResultStream = filterAdClickStream.keyBy(AdClickEvent::getProvince)
                .timeWindow(Time.hours(1), Time.minutes(5)) // 定义划窗，5分组输出一次
    ...
    public static class FilterBlackListUser extends KeyedProcessFunction<Tuple, AdClickEvent, AdClickEvent> {
        ...
        public void open(Configuration parameters) {
            countState = getRuntimeContext().getState(new ValueStateDescriptor<>("ad-count", Long.class, 0L));
            isSentState = getRuntimeContext().getState(new ValueStateDescriptor<>("is-sent", Boolean.class, false));
        }

        @Override
        public void processElement(AdClickEvent value, Context ctx, Collector<AdClickEvent> out) throws Exception {
            // 判断当前用户对同一广告的点击次数，如果不够上限，就count+1，正常输出，如果达到上限，直接过滤掉，并侧输出流输出黑名单报警
            // 首先获取当前的count值
