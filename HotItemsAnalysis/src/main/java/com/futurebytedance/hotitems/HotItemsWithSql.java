 ...
 * @version 1.0
 * @date 2021/5/3 - 15:09
 * @Description 实时热门商品统计 - Flink SQL实现
 */
public class HotItemsWithSql {
    public static void main(String[] args) throws Exception {
        ...
        // 7.利用开窗函数，对count值进行排序并获取Row number，得到TopN
        // SQL
        ...
        Table resultTable = tableEnv.sqlQuery("select * from (select *,ROW_NUMBER() OVER(partition by windowEnd order by cnt desc) as row_num from agg) a where row_num <= 5");

        // 纯SQL实现
        tableEnv.createTemporaryView("data_table", dataStream, "itemId,behavior,timestamp.rowtime as ts");
        Table resultSqlTable = tableEnv.sqlQuery("select * from (select *,ROW_NUMBER() OVER(partition by windowEnd order by cnt desc) as row_num from (" +
                "select itemId,count(itemId) as cnt,HOP_END(ts,interval '5' minute,interval '1' hour) as windowEnd from data_table where behavior = 'pv' group by itemId," +
